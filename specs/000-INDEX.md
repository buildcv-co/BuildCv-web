...
0. **022-llm-integration-local-mvp (v0.5.4, cross-repo: web panel + api base)** — 🚧 **EN CURSO — READY FOR sdd-verify**. Optional local-first LLM feedback (fake provider default; Ollama deferred a 023). Status: PR1 API MERGED ✅ (`BuildCv-api/main @ 85c85de`, contracts + fake provider), PR2 API MERGED ✅ (`BuildCv-api/main @ 00f64ed`, `POST /api/v1/llm/feedback` + PII redaction + dedicated rate limit + sanitized logs + timeout/provider degraded fallback; production LOC 308 ≤ 400 cap; 25 new tests; Art. II preserved `ScoringEngine.Version=2.0.0`), PR3 WEB MERGED ✅ (`BuildCv-web/main @ 7009fdf`, BFF `POST /api/llm/feedback` → backend `/api/v1/llm/feedback`, typed `lib/api/llm.ts` adapter, endpoint drift updated; production LOC 264 ≤400; 17 new Vitest tests; fresh review APPROVE), and PR4 WEB MERGED ✅ (`BuildCv-web/main @ bb81e02`, separate analyzer feedback panel + sessionStorage toggle + 9 states + a11y basics + fake e2e; production LOC 218 ≤400; 14 automated tests; fresh review APPROVE). Pendiente: sdd-verify, sdd-archive + tag final. **Nota baseline**: `/adapt` `RequireCreditsFilterTests.Adapt_without_jwt_returns_401` falla IGUAL en `BuildCv-api/main` pre-PR1 (reproducido independientemente en commit `496a3c7`); NO es regresión de 022, documentada en `BuildCv-web/specs/009-auth-web/verify-report.md` como parte de las 34 baseline integration failures. Follow-up separado (`024-adapt-credits-fix` o similar) post-MVP si se prioriza. Artifacts: [./022-llm-integration-local-mvp/](./022-llm-integration-local-mvp/). **Recommended NEXT step**: `sdd-verify 022`.
