...
0. **022-llm-integration-local-mvp (v0.5.4, cross-repo: web panel + api base)** — 🚧 **EN CURSO**. Optional local-first LLM feedback (fake provider default; Ollama deferred a 023). Status: PR1 API MERGED ✅ (`BuildCv-api/main @ 85c85de`, scope: `LlmFeedback` contracts + port + `FakeLlmFeedbackClient` determinista + `LlmFeedbackOptions` defaults; defaults `Enabled=false`, `Provider=fake`, `Model=fake-local-v1`; production LOC 248 ≤ 400 cap; 10 tests; 0 endpoint creado; Art. II preserved `ScoringEngine.Version=2.0.0`). Pendiente: PR2 (endpoint + PII redaction + rate limit + fallback + timeout, api), PR3 (BFF + adapter, web), PR4 (panel + toggle + a11y + e2e fake, web), sdd-verify, sdd-archive + tag final. **Nota baseline**: `/adapt` `RequireCreditsFilterTests.Adapt_without_jwt_returns_401` falla IGUAL en `BuildCv-api/main` pre-PR1 (reproducido independientemente en commit `496a3c7`); NO es regresión de 022, documentada en `BuildCv-web/specs/009-auth-web/verify-report.md` como parte de las 34 baseline integration failures. Follow-up separado (`024-adapt-credits-fix` o similar) post-MVP si se prioriza. Artifacts: [./022-llm-integration-local-mvp/](./022-llm-integration-local-mvp/). **Recommended NEXT step**: `sdd-apply 022 PR2`.
