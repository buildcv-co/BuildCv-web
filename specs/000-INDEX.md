...
**Última actualización:** 2026-06-26 (v1: **009-auth-web** 🚧 **EN CURSO** — api PR0 MERGED ✅ + web PR1 MERGED ✅ + web PR2 MERGED ✅. PR0: `POST /api/v1/auth/web-signup` (body `{provider, providerAccountId, email, name}`) + BFF auth via `BffCredentialFilter` + `IRefreshTokenStore.RevokeAllForUserAsync` + bearer-only logout. PR1: web auth adapter + contract fix + MINOR-1 fix (skip+warn si name empty per Art. I). PR2: 3 BFF routes server-side (`/api/auth/session`, `/api/auth/refresh`, `/api/auth/logout`) + 2 client helpers (`lib/api/session.ts`, `lib/api/sign-out.ts`) + cache cleanup + idempotent sign-out + `SessionExpiredError` controlada. **MVP Auth Readiness Checkpoint + triage PR0 open notes** en apply-progress.md. Verdicts: PR0 + Patch A → APPROVE_WITH_MINOR_NOTES; PR1 fresh review → APPROVE_WITH_MINOR_NOTES; PR1 MINOR-1 re-review → APPROVE; PR2 fresh review → APPROVE_WITH_MINOR_NOTES (size decision ACCEPT_SIZE_DEVIATION, 417 > 350 cap pero dentro budget 400). Web PR3-PR8 pending. **MVP launch path**: next PR4 (/cuenta skeleton + GET user-data BFF), then PR6 (ARCO UI per Art. IX Habeas Data), then PR7 (UserMenu), then PR8 (a11y + OpenAPI CI). PR3 (/privacidad) y PR5 (consent UI) safe-defer post-MVP. PR0 hardening (3 items SHOULD_FIX_BEFORE_LAUNCH) puede ir como hotfix entre PR6 y PR7. · v0.5.3: **021-structured-cv-import-and-job-input** ✅ **SHIPPED + ARCHIVED** — Engine bump MAYOR `1.0.0 → 2.0.0` (Constitution Art. II SemVer seal, nota agregada en `constitution.md`); JSON Resume `CvDocument` + `ConfidenceMarker` en parsers; `JobSpec` mandatory con Zod; editor open-resume-inspired (`BasicsForm`/`WorkList`/`EducationList`/`SkillsByCategory`); `promoteConfidence` solo en editor on blur (Constitution Art. I); `engineVersion: "2.0.0"` (Art. II seal, response observation tagging via `engineVersion` en `lib/observability`); feature flag `NEXT_PUBLIC_STRUCTURED_INPUT=true` (default en `.env.example` + `.env.local`); 6 chained PRs + 1 followup (PR1 foundation → PR2 parser restructure → PR3 scoring v2 → PR4 web editor → PR5 web analyzer + observability → PR6 tests/a11y/e2e/docs → followups-1 fixing 8 e2e tests + AnalizarScreen bug + typo fix); 23 work-unit commits web (`30672b4` → `b75c5b1` + `55985f3` + typo fix `82a400b`) + 11 work-unit commits api (`5f3982a` → `6f0456f`) all merged to `main` + pushed to `origin` in BOTH repos; HEAD web `82a400b`, HEAD api `5d40a53`; delta specs synced into main `BuildCv-web/specs/{006-web-cv-editor,008-observability-web}/spec.md` + `BuildCv-api/specs/{002-score-engine,005-cv-pdf-docx-import}/spec.md`; see [archive report](./021-structured-cv-import-and-job-input/archive-report.md))
...
| 009 | `auth-web` | v1 | 🚧 **EN CURSO** (api PR0 MERGED ✅ + web PR1 MERGED ✅ + web PR2 MERGED ✅; web PR3-PR8 pending). MVP launch path: next PR4 → PR6 → PR7 → PR8. | (n/a) |
...
- **Estado:** 🚧 **EN CURSO** (api PR0 MERGED ✅ + web PR1 MERGED ✅ + web PR2 MERGED ✅). **MVP Auth Readiness Checkpoint** presente en apply-progress.md: READY FOR MVP if PR4 + PR6 also land. PR6 ARCO UI es **MVP_BLOCKER** per Constitution Art. IX (Habeas Data). PR2 fresh review verdict: APPROVE_WITH_MINOR_NOTES (0 BLOCKER, 0 MAJOR, 2 MINOR + 2 NIT; size decision **ACCEPT_SIZE_DEVIATION** con 6 razones documentadas, key being cohesion of session/refresh/logout as single auth-state MVP slice; 417 > 350 cap pero dentro de budget formal 400). **Triage PR0 open notes (7 items)**: 3 SHOULD_FIX_BEFORE_LAUNCH (logout 500 vs 401; no test for missing `providerAccountId`; permissive email regex); 4 SAFE_DEFER_POST_MVP (missing OpenAPI `.Accepts/.Produces`; `_providerKeyMap` bug pre-existing; T-PR0-007 tracking gap; X-BFF-Key no documentado en OpenAPI).
...
- **PR2 web commits (4, autosquash clean, merged to `web/main`):** `4cfefb9` (test auth: cubrir refresh y cierre de sesión) → `e9ff2b3` (fix auth: agregar helpers de sesión y sign-out) → `ab8e83e` (docs 009-auth-web: registrar avance PR2 + MVP Auth Readiness Checkpoint) → `849ae89` (docs 009-auth-web: cerrar MINOR-1 PR2 honest size deviation + commit fresh review). **Merged to `web/main` via `--no-ff`** preserving branch history. Suite 1066/1066 (was 1042 pre-PR2 = +24 net new). 7 pre-existing typecheck errors unchanged.
- **Required env vars for PR3+:** web BFF must include `X-BFF-Key: process.env.BFF_API_KEY` header in every request to backend. `BFF_API_KEY` (web env) must match `Auth__BffApiKey` (api env). PR1 added `BFF_API_KEY` placeholder to `.env.example`.
- **MVP launch path documented:** next PR4 (`/cuenta` skeleton + GET user-data BFF, ~175 LOC, 8 tests) → PR6 (ARCO UI per Art. IX Habeas Data, ~300 LOC, 12 tests) → PR7 (`<UserMenu>`, ~175 LOC, 8 tests) → PR8 (e2e + a11y hardening + OpenAPI CI gate, ~250 LOC, 15 tests). PR3 (`/privacidad` page) y PR5 (consent UI) **SAFE_DEFER_POST_MVP**. PR0 hardening (3 SHOULD_FIX_BEFORE_LAUNCH items) puede ir como hotfix entre PR6 y PR7.
- **Web PR3-PR8 status (pending):** PR3 `/privacidad` SAFE_DEFER; PR4 `/cuenta` NEXT for MVP; PR5 consent SAFE_DEFER; PR6 ARCO MVP_BLOCKER after PR4; PR7 UserMenu SHOULD_FIX; PR8 e2e/a11y SHOULD_FIX.
