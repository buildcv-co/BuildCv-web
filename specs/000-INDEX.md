...
| 009 | `auth-web` | v1 | 🚧 **EN CURSO** (api PR0 MERGED ✅ + web PR1 MERGED ✅; web PR2-PR8 pending) | (n/a) |
...
- **Estado:** 🚧 **EN CURSO** (api PR0 MERGED ✅ + web PR1 MERGED ✅). SDD planning completo (explore + propose + spec + design + tasks). sdd-apply PR0 ejecutado en api backend con `POST /api/v1/auth/web-signup` + `IRefreshTokenStore.RevokeAllForUserAsync` + bearer-only logout + `BffCredentialFilter` (BFF auth). sdd-apply PR1 ejecutado en web con auth adapter (`lib/api/auth-adapter.ts`, 108 LOC) + BFF route (`app/api/auth/web-signup/route.ts`, 73 LOC) + `lib/auth.ts` MINOR-1 fix (skip+warn si name empty per Art. I, no inventar email local-part). PR0 verdict: APPROVE_WITH_MINOR_NOTES (0 BLOCKER, 0 MAJOR post-patch, 5 MINOR + 2 NIT accepted para follow-up: logout 500 vs 401; missing OpenAPI `.Accepts/.Produces`; no test for missing providerAccountId; pre-existing `_providerKeyMap` bug; T-PR0-007 tracking gap; permissive email regex; X-BFF-Key no documentado en OpenAPI). PR1 fresh review verdict: APPROVE_WITH_MINOR_NOTES (0 BLOCKER, 0 MAJOR, 1 MINOR + 2 NIT: events.signIn fallback `name=""`; apply-progress count drift 8→7; `7888b2c fixup!` autosquash before merge). PR1 MINOR-1 re-review verdict: APPROVE (MINOR-1, NIT-1, NIT-2 CLOSED ✅).
...
- **PR1 web commits (5, autosquash + follow-up, merged to `web/main`):** `2736e8a` (test auth: cubrir contrato web-signup, autosquash de `b7ae3e6 + 7888b2c`) → `5fbf47c` (fix auth: adaptar web signup al contrato bff) → `c75bfd0` (docs 009-auth-web: registrar avance PR1) → `3ef7146` (fix auth: validar nombre antes de web-signup, MINOR-1 closure) → `be2f257` (docs 009-auth-web: registrar cierre MINOR-1 + NIT-1 PR1 review follow-up). **Merged to `web/main` via `--no-ff`** preserving branch history (5 atomic commits).
- **Required env vars for PR2+:** web BFF must include `X-BFF-Key: process.env.BFF_API_KEY` header in every POST to `BACKEND_URL/api/v1/auth/web-signup`. `BFF_API_KEY` (web env) must match `Auth__BffApiKey` (api env). PR1 added `BFF_API_KEY` placeholder to `.env.example`.
- **Web PR2-PR8 pending:** PR2 session refresh + sign-out helpers (~125 LOC, 8 tests); PR3 `/privacidad` + version selector (~175 LOC, 9 tests); PR4 `/cuenta` skeleton + GET user-data BFF (~175 LOC, 8 tests); PR5 consent management (~300 LOC, 12 tests); PR6 ARCO request flow (~300 LOC, 12 tests); PR7 `<UserMenu>` (~175 LOC, 8 tests); PR8 e2e + a11y hardening + OpenAPI CI gate deferred from PR1 (~250 LOC, 15 tests). Total web PRs PR2-PR8: ~1570 LOC, ~80 tests (PR1 took ~30 tests vs forecast 10).
