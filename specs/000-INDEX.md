...
| 009 | `auth-web` | v1 | ✅ **SHIPPED + ARCHIVED** — verify `PASS_WITH_NOTES`; tag `009-auth-web-v1.0` en SHIP commits web `9f71e9f` / api `66fcaf1`; MVP scope web PR1+PR2+PR4+PR6a+PR6b+PR7+PR8+NIT+verify-fixes + api PR0+hardening; PR3 `/privacidad` y PR5 consent UI diferidos post-MVP. See [archive report](./009-auth-web/archive-report.md). | (n/a) |
...
| 022 | `llm-integration-local-mvp` (cross-repo: optional local-first LLM feedback) | v0.5.4 | ✅ **SHIPPED + ARCHIVED** — verify `PASS_WITH_NOTES`; tag `022-llm-integration-local-mvp-v1.0` created at SHIP commit web `bb81e02` / api `00f64ed` (not archive commits). MVP scope: PR1+PR2 api (contracts, fake provider, endpoint, PII redaction, sanitized logs, dedicated rate limit, timeout/fallback) + PR3+PR4 web (BFF `/api/llm/feedback`, `lib/api/llm.ts`, separate analyzer feedback panel, sessionStorage toggle, 9 states, a11y basics, fake e2e). PR5 Ollama/provider work deferred to 023; 009 `/privacidad`/consent and production smoke remain SAFE_DEFER_POST_MVP. See [archive report](./022-llm-integration-local-mvp/archive-report.md). | `app/api/llm/feedback/route.ts` |
| 024 | `minimax-real-provider` (cross-repo: optional MiniMax cloud feedback provider) | v0.5.5 | ✅ **SHIPPED + ARCHIVED** — verify `PASS_WITH_NOTES`; tag `024-minimax-real-provider-v1.0` created at SHIP commits web `1a0a563` / api `98995c3` (not archive commits). MVP scope: PR1+PR2 api (`MinimaxLlmFeedbackClient`, options/caps, conditional DI, handler dispatch, max-input/output, sanitized error mapping, endpoint tests) + PR3 web type-only contract fix (`provider: "fake" | "minimax"`, `model: string`), 3 PRs cross-repo. PR5/Ollama, persistent preference, streaming, thinking, billing, deploy automation, 009 privacy/consent, 010 payments, and 020 a11y remain SAFE_DEFER_POST_MVP. READY FOR sdd-archive done. See [archive report](./024-minimax-real-provider/archive-report.md). | `app/api/llm/feedback/route.ts` |
...
- **Verify:** [specs/009-auth-web/verify-report.md](./009-auth-web/verify-report.md) (`PASS_WITH_NOTES`)
- **Archive:** [specs/009-auth-web/archive-report.md](./009-auth-web/archive-report.md)
...
- **Estado:** ✅ **SHIPPED + ARCHIVED**. Verify verdict `PASS_WITH_NOTES`; MVP_BLOCKER 0; SHOULD_FIX_BEFORE_LAUNCH 0; READY_FOR_TAG yes. Final tag `009-auth-web-v1.0` apunta a los SHIP commits web `9f71e9f788f59f0fdc687b71d2b70629145eb1ad` y api `66fcaf1a13d511eb088ae93443f255c376459ebf`, no a los commits de archivo. MVP scope shipped: api PR0 + PR0 hardening; web PR1 + PR2 + PR4 + PR6a + PR6b + PR7 + PR8 + NIT cleanup + verify-fixes. PR3 `/privacidad` y PR5 consent UI quedan SAFE_DEFER_POST_MVP.
...
- **PR1 web commits (5, autosquash + follow-up, merged to `web/main`):** `2736e8a` (test auth: cubrir contrato web-signup, autosquash de `b7ae3e6 + 7888b2c`) → `5fbf47c` (fix auth: adaptar web signup al contrato bff) → `c75bfd0` (docs 009-auth-web: registrar avance PR1) → `3ef7146` (fix auth: validar nombre antes de web-signup, MINOR-1 closure) → `be2f257` (docs 009-auth-web: registrar cierre MINOR-1 + NIT-1 PR1 review follow-up). **Merged to `web/main` via `--no-ff`** preserving branch history.
- **PR2 web commits (4, merged to `web/main`):** `4cfefb9` (test auth: cubrir refresh y cierre de sesión) → `e9ff2b3` (fix auth: agregar helpers de sesión y sign-out) → `ab8e83e` (docs 009-auth-web: registrar avance PR2 + MVP Auth Readiness Checkpoint) → `849ae89` (docs 009-auth-web: cerrar MINOR-1 PR2 honest size deviation + commit fresh review). **Merged to `web/main` via `--no-ff`** preserving branch history. Suite 1066/1066.
- **PR4 web commits (5, merged to `web/main`):** `a6fed6b` (test cuenta: cubrir bff de datos de usuario) → `8c3e641` (feat cuenta: agregar skeleton y carga de datos) → `866c1b1` (docs 009-auth-web: registrar avance PR4 + MVP Auth+Account Readiness Checkpoint) → `c94fddf` (docs 009-auth-web: corregir aritmética de desviación PR4 + addendum review MINOR-1) → merge commit. **Merged to `web/main` via `--no-ff`** preserving branch history. Suite 1089/1089. Production LOC 714 verified (over forecast +539, over cap +364, accepted with SIZE_DEVIATION). 16 files changed, 2246 insertions in merge.
- **Required env vars for PR5+:** web BFF must include `X-BFF-Key: process.env.BFF_API_KEY` header in every request to backend. `BFF_API_KEY` (web env) must match `Auth__BffApiKey` (api env).
- **MVP launch path documented:** next PR6 (ARCO UI per Art. IX Habeas Data, ~300 LOC, 12 tests) → PR7 (`<UserMenu>`, ~175 LOC, 8 tests) → PR8 (e2e + a11y hardening + OpenAPI CI gate, ~250 LOC, 15 tests). PR3 (`/privacidad` page) y PR5 (consent UI) **SAFE_DEFER_POST_MVP**. PR0 hardening (3 SHOULD_FIX_BEFORE_LAUNCH items) puede ir como hotfix entre PR6 y PR7.
- **Web PR3+PR5-PR8 status (pending):** PR3 `/privacidad` SAFE_DEFER; PR5 consent SAFE_DEFER; PR6 ARCO **MVP_BLOCKER next**; PR7 UserMenu SHOULD_FIX; PR8 e2e/a11y SHOULD_FIX.
...
0. ~~**022-llm-integration-local-mvp (v0.5.4, cross-repo: web panel + api base)**~~ → ✅ **SHIPPED + ARCHIVED**. Verify `PASS_WITH_NOTES`; tag `022-llm-integration-local-mvp-v1.0` at SHIP commits web `bb81e02` / api `00f64ed` (not archive commits). MVP shipped in 4 PRs: PR1+PR2 api (contracts, fake provider, endpoint, PII redaction, sanitized logs, dedicated rate limit, timeout/fallback) + PR3+PR4 web (BFF `/api/llm/feedback`, `lib/api/llm.ts`, separate analyzer feedback panel, sessionStorage toggle, 9 states, a11y basics, fake e2e). `MVP_BLOCKER=0`, `SHOULD_FIX_BEFORE_LAUNCH=0`, `SAFE_DEFER_POST_MVP=7`. **Nota baseline**: `/adapt` `RequireCreditsFilterTests.Adapt_without_jwt_returns_401` falla IGUAL en `BuildCv-api/main` pre-PR1 (`496a3c7`); NO es regresión de 022. Archive report: [./022-llm-integration-local-mvp/archive-report.md](./022-llm-integration-local-mvp/archive-report.md). **Recommended NEXT step**: 023 Ollama, 010 payments web, 020 a11y audit, or deploy MVP.
