...
| 009 | `auth-web` | v1 | 🚧 **EN CURSO** (api PR0 MERGED ✅ + web PR1 MERGED ✅ + web PR2 MERGED ✅ + web PR4 MERGED ✅; web PR3+PR5-PR8 pending). MVP launch path: next PR6 (ARCO UI, MVP_BLOCKER per Art. IX) → PR7 → PR8. | (n/a) |
...
- **Estado:** 🚧 **EN CURSO** (api PR0 MERGED ✅ + web PR1 MERGED ✅ + web PR2 MERGED ✅ + web PR4 MERGED ✅). **MVP Auth + Account Readiness Checkpoint** presente en apply-progress.md: READY FOR PR6. PR6 ARCO UI es el único **MVP_BLOCKER** per Constitution Art. IX (Habeas Data). PR2 fresh review verdict: APPROVE_WITH_MINOR_NOTES (size deviation ACCEPT, 417 > 350 cap). PR4 fresh review verdict: APPROVE_WITH_MINOR_NOTES (0 BLOCKER, 0 MAJOR, 1 MINOR + 2 NIT; MINOR-1 closed via doc arithmetic fix). **4 deviations accepted**: SIZE_DEVIATION, SLOT_PLACEHOLDERS, SHARED_UTILS, COPY_EXPANSION. **Triage PR0 open notes (7 items)**: 3 SHOULD_FIX_BEFORE_LAUNCH (logout 500 vs 401; no test for missing `providerAccountId`; permissive email regex); 4 SAFE_DEFER_POST_MVP (missing OpenAPI `.Accepts/.Produces`; `_providerKeyMap` bug pre-existing; T-PR0-007 tracking gap; X-BFF-Key no documentado en OpenAPI).
