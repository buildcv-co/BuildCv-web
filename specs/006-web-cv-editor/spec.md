...
> **Source delta**: [`./specs/006-web-cv-editor/spec.md`](./specs/006-web-cv-editor/spec.md)
...
The system MUST honor `NEXT_PUBLIC_STRUCTURED_INPUT` (default `"true"` post-021-archive per .env.example; rollback = flip to `"false"` instant, no redeploy needed). When `false`, the legacy editor (8 textareas, MD round-trip, `{ cvText, jobText }` payloads) is rendered. When `true`, the new JSON-Resume-shaped editor is rendered.
